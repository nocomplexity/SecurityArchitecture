
% This file is generated automatically. If you have a suggestion, simply open an issue in the GitHub repository, and we will consider adding it to the list. The tools used to build this list are FOSS (Free and Open Source Software). Check the tools on https://nocomplexity.com/ or my github.com/nocomplexity to use it too! Its FOSS. 

# The Beginner's Textbook for Fully Homomorphic Encryption


<br />

| Book Info | Value |
| --- | --- |
| Author | Robin Geelen e.a. |
| Description | Fully Homomorphic Encryption (FHE) is a cryptographic scheme that enables computations to be performed directly on encrypted data, as if the data were in plaintext. Read this book if you want to learn the math behind it! Found on https://arxiv.org/abs/2503.05136 |
| Notice |  -  |
| Repository | N.A. |

<br /><br />
::::{tab-set} 
:::{tab-item} Online version 

```{button-link} https://fhetextbook.github.io/ 
:ref-type: ref 
:color: danger 
Link to Online Version »»
```


:::
:::{tab-item} PDF/ePUB version 

```{button-link} https://arxiv.org/pdf/2503.05136 
:ref-type: ref 
:color: danger 
Link to PDF and/or ePUB version
```


:::
:::{tab-item} Hardcopy version
No hardcopy version available.

:::
::::


% This file is generated automatically. If you have a suggestion, simply open an issue in the GitHub repository, and we will consider adding it to the list. The tools used to build this list are FOSS (Free and Open Source Software). Check the tools on https://nocomplexity.com/ or my github.com/nocomplexity to use it too! Its FOSS. 

